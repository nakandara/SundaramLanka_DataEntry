...
const ProductionList = (props) => {
  const { restaurants, setRestaurants, addSucess,hold,setHold } = useContext(
    RestaurantsContext
  );
const [refresh, setRefresh] = useState(false)
  ...
  const fetchData = async () => {
    try {
      const response = await RestaurantFinder.get("/reporting/hold/");
      
      setHold(response.data.data.data);
  ...
  const handleUpdate = (e, id) => {
    e.stopPropagation();
    history.push(`hold/${id}/update/hold`);
  };
            ...
            hold.map((d,index) => {
              return (
                ...
                >
                  ...
                  <td className='wrap'>{d.hold_compound}</td>
                  <td>{d.reject_compound}</td>
