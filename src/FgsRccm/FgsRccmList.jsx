...
const QualityRccmList = (props) => {
  const { restaurants, setRestaurants, addSucess ,qualityRccm,setQualityRccm} = useContext(
    RestaurantsContext
  );
const [refresh, setRefresh] = useState(false)
  ...
  const fetchData = async () => {
    try {
      const response = await RestaurantFinder.get("/reporting/qualityy/rccm");

      console.log(response.data.data.data);
      
      setQualityRccm(response.data.data.data);
  ...
  const handleUpdate = (e, id) => {
    e.stopPropagation();
    history.push(`/quality/rccm/update/${id}`);
  };
...
export default QualityRccmList;
