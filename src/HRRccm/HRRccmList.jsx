...
const HRRccmList = (props) => {
  const { restaurants, setRestaurants, addSucess ,hrRccm,setHRRccm} = useContext(
    RestaurantsContext
  );
const [refresh, setRefresh] = useState(false)
  ...
  const fetchData = async () => {
    try {
      const response = await RestaurantFinder.get("/reporting/hrd/rccm");

      console.log(response.data.data.data);
      
      setHRRccm(response.data.data.data);
  ...
  const handleUpdate = (e, id) => {
    e.stopPropagation();
    history.push(`/hr/rccm/update/${id}`);
  };
