...
const ProductionRccmList = (props) => {
  const { restaurants, setRestaurants, addSucess ,productionRccm,setProductionRccm} = useContext(
    RestaurantsContext
  );
const [refresh, setRefresh] = useState(false)
  ...
  const fetchData = async () => {
    try {
      const response = await RestaurantFinder.get("/reporting/production/rccm");

      console.log(response.data.data.data);
      
      setProductionRccm(response.data.data.data);
  ...
  const handleUpdate = (e, id) => {
    e.stopPropagation();
    history.push(`/production/rccm/update/${id}`);
  };
...
export default ProductionRccmList;
