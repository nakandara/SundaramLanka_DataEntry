...
const ProductionList = (props) => {
  const { restaurants, setRestaurants, addSucess,production,setProduction } = useContext(
    RestaurantsContext
  );
const [refresh, setRefresh] = useState(false)
  ...
  const fetchData = async () => {
    try {
      const response = await RestaurantFinder.get("/reporting/mom/");
      
      setProduction(response.data.data.data);
  ...
  const handleUpdate = (e, id) => {
    e.stopPropagation();
    history.push(`production/${id}/update/production`);
  };

  const handleRestaurantSelect = (id) => {
    history.push(`production/mom/${id}`);
  };
      ...
      <table className="table table-hover table-dark"   >
        <thead>
          <tr className="bg-primary">
            ...
            <th scope="col">Plant Utilization</th>
            <th scope="col"> Plan Adherence</th>
            <th scope="col">Actual Weight</th>
            <th scope="col">Plan Weight</th>
            ...
            production.map((d,index) => {
              return (
                ...
                >
                  ...
                  <td className='wrap'>{d.plant_utilization}</td>
                  <td>{d.plan_adherence}</td>
                  <td>{d.actual_weight}</td>
                  <td>{d.plan_weight}</td>
