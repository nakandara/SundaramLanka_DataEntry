...
const AddBeadRccm = () => {
  ...
  const { addBeadRccm } = useContext(RestaurantsContext);
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('ok');
    try {
      const response = await RestaurantFinder.post("/reporting/bead/rccm", {
        in_date,
        ap_no,
        accountability, 
        metix,
      ...
      addBeadRccm(response.data.data.data);
    } catch (err) {
      console.log(err);
    }
  }; 
  ...
  return (
    <div className="mb-4" style={{margin:'30px'}}>
      <form action="">
