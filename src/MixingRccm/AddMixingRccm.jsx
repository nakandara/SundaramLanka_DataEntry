...
const AddMixingRccm = () => {
  ...
  const { addMixingRccm } = useContext(RestaurantsContext);
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('ok');
    try {
      const response = await RestaurantFinder.post("/reporting/mixing/rccm", {
        in_date,
        ap_no,
        accountability, 
        metix,
      ...
      addMixingRccm(response.data.data.data);
    } catch (err) {
      console.log(err);
    }
  }; 
    ...
    <div className="mb-4">
      <form action="">
        <div className="form-GROUP">
          ...
          <div className="col m-1">
          <select  aria-label="Default select example"  onChange={(e)=>handleOption(e)} className="form-control">
          <option selected>Category</option>
         <option value="compound_weight_actual"   >Compound Weight</option>
           <option value="mixing_break_down"  >Break Down</option>
       <option value="mixing_productivity"  >Productivity</option>
      </select> 
      </div>
