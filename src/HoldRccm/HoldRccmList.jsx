...
const HoldRccmList = (props) => {
  const { restaurants, setRestaurants, addSucess ,holdRccm,setHoldRccm} = useContext(
    RestaurantsContext
  );
const [refresh, setRefresh] = useState(false)
  ...
  const fetchData = async () => {
    try {
      const response = await RestaurantFinder.get("/reporting/holdand/rccm");

      console.log(response.data.data.data);
      
      setHoldRccm(response.data.data.data);
  ...
  const handleUpdate = (e, id) => {
    e.stopPropagation();
    history.push(`/hold/rccm/update/${id}`);
  };
